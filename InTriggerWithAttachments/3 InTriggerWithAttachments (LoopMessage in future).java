...
trigger SObjectTrigger on SObject__c (after update) {
    ...
    Map<Id, SObject__c> mySObjects = new Map<Id, SObject__c>([
        SELECT Id, Contact__c, Contact__r.AccountId, (SELECT Id FROM Attachments WHERE Name LIKE 'AB-%')
        FROM SObject__c
...
private class SObjectTriggerTest {
    private static testMethod void testSObjectTrigger() {
        ...
        Attachment a = new Attachment(name='test', parentId=so.Id, body=Blob.valueOf('test'));
        insert a;
